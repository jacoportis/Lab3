...
Per calcolare l'attenuazione a un esatto valore di frequenza si possono utilizzare i cosiddetti nomogrammi, grafici in cui vengono riportate lunghezza, attenuazione e frequenza sulle loro scale. Conoscendo due grandezze, possiamo ricavare la terza attraverso un nomogramma unendo i due valori e andando a vedere l'intercetta nella terza scala.
...
\subsection{Banda passante di un oscilloscopio}
...
Lo stadio di ingresso\footnote{Lo stadio di ingresso di un oscilloscopio è la parte iniziale del circuito dove viene applicato il segnale da misurare.} può essere in diverse modalità: 
\begin{itemize}[leftmargin=0.5cm]
...
Inoltre l'oscilloscopio digitale, proprio per il fatto che digitalizza l'informazione trasformandola in dati che possiamo salvare, ci permette di fare delle operazioni, quindi non solo lavorare sul trigger ma anche di fare delle analisi dei segnali in tempo in tempo (ad esempio si possono produrre degli spettri per i modelli più avanzati), quindi in generale ci sono delle funzionalità che quello analogico non possiede.