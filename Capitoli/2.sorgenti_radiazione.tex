...
\begin{itemize}[leftmargin=0.5cm]
    \item Particelle cariche pesanti (protoni, $\alpha$, ioni pesanti);
    \item Elettroni.
\end{itemize}
...
Ricordiamo che la massa dell'elettrone è pari a $0.511 \; \rm MeV$ mentre la massa del protone è dell'ordine del $\rm GeV$, dunque tra i due c'è un fattore $2 \cdot 10^3$.
...
\begin{itemize}[leftmargin=0.5cm]
    \item Sorgenti radioattive: da pochi $\rm eV$ (quindi poco energetiche) a $\rm 10^7 \; eV(=10 \; MeV)$;
...
Entrambe le grandezze hanno le dimensioni di un tempo, dunque si misurano in secondi. In particolare la vita media corrisponde al tempo necessario affinché il numero di nuclei si riduca di un fattore $e^{-1}$, cioè il tempo per passare da $N_0$ a $N_0/e$, l'emivita invece corrisponde al tempo necessario affinché il numero di nuclei di partenza si dimezzi, cioè il tempo per passare da $N_0$ a $N_0/2$. La relazione con la costante di decadimento si ricava tramite semplici passaggi matematici: imponendo $N(t)=N_0/2$ si ha che
...
Vediamo uno schema dei livelli delle sorgenti (in laboratorio adopereremo \ce{^{60}Co} e \ce{^{137}Ce}):
