...
\begin{itemize}[leftmargin=0.5cm]
   ...
   \item La tensione di alimentazione è di alcune centinaia di Volt, però i guadagni non sono elevatissimi, dell'ordine di $50-100$;
...
\end{itemize}
Tuttavia ancora si deve lavorare un po' sulle dimensioni le quali sono ancora piccole e sul dark count rate, cioè quel rumore che è sempre presente nei fotosensori anche in assenza di radiazione, che in questo caso può essere anche molto elevato (tra $\rm 100 \; kHz/mm^2$ e $\rm 1 \; MHz/mm^2$), dipende dal produttore.
...
\vspace{-0.1cm}Avremo quindi che il primo picco è quello corrispondente al primo fotoelettrone, il secondo picco è quello corrispondente a due fotoelettroni e così via.
...
L'efficienza quantica intrinseca è più elevata rispetto a quella che abbiamo visto nel caso dei fotomoltiplicatori tradizionali, potendo anche assumere valori superiori all'80\%,\footnote{L'efficienza quantica intrinseca si riferisce solo alla probabilità che un fotone assorbito generi effettivamente una coppia elettrone--lacuna (o un elettrone libero). Con essa non si considerano le perdite ottiche (ad esempio, riflessione superficiale o trasmissione attraverso il dispositivo).\comment{È una misura della qualità del materiale nel convertire energia luminosa in portatori di carica utili, una volta che il fotone è stato assorbito.}} a seconda del tipo di sensore e dalla lunghezza d'onda che andiamo a rivelare.
...
\begin{center}
   \begin{tabular}{|c|c|c|c|}
      ...
      \hline
      &&&\\[-0.45cm]
      Guadagno & $10^6-10^7$ & $\sim 100$ & $10^5-10^6$\\
      \hline
      &&&\\[-0.45cm]
